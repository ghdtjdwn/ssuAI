...
1. GitHub에서 ssuAI repository 접속
2. 상단 `Actions` 탭 클릭
3. `CI` workflow 클릭
...
용도: k3s가 `ghcr.io/ghdtjdwn/ssuai-backend:latest` image를 pull할 수 있게 하기 위함입니다.
...
- [ ] `ghcr.io/ghdtjdwn/ssuai-backend` package가 보임
...
```powershell
curl.exe -N -D - https://<BACKEND_HOST>/sse
```

정상이라면 connection이 열린 채 유지될 수 있습니다. 확인 후 `Ctrl+C`로 종료합니다.

체크:

- [ ] `Content-Type: text/event-stream` 확인
- [ ] `/mcp/message?sessionId=...` 비슷한 endpoint data 확인
...
```text
https://ssuai-backend.onrender.com
```

Render MCP endpoint:

```text
https://ssuai-backend.onrender.com/sse
```
