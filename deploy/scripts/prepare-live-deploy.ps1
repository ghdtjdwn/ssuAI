param(
    ...
    [string]$Image = "ghcr.io/ghdtjdwn/ssuai-backend:latest",
...
function Split-TaggedImage {
    ...
    if ($lastColon -le $lastSlash -or $lastColon -eq ($TaggedImage.Length - 1)) {
        throw "Image must include a tag, for example ghcr.io/ghdtjdwn/ssuai-backend:sha-<full-sha>. Received: $TaggedImage"
    }
