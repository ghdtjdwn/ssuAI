...
- Backend image registry at `ghcr.io/ghdtjdwn/ssuai-backend`.
...
| GitHub PAT | GitHub UI | Fine-grained PAT scoped to `contents:write` on `ghdtjdwn/ssuAI` only, used by Image Updater. |
...
`ghcr.io/ghdtjdwn/ssuai-backend:sha-<full-sha>` on every push to `main`.
...
1. Push to `main`.
2. CI builds and pushes `ghcr.io/ghdtjdwn/ssuai-backend:sha-<full-sha>`.
...
After CI's `image-build` job publishes `ghcr.io/ghdtjdwn/ssuai-backend:sha-<full-sha>`:

```bash
SHA=$(git rev-parse origin/main)
sudo kubectl set image deployment/ssuai-backend \
  backend=ghcr.io/ghdtjdwn/ssuai-backend:sha-${SHA} \
  -n ssuai-prod
...
powershell -ExecutionPolicy Bypass -File deploy/scripts/prepare-live-deploy.ps1 `
  ...
  -Image ghcr.io/ghdtjdwn/ssuai-backend:sha-<full-sha>
```
