...
- **ssuMCP 배포 확인**: TROUBLESHOOTING.md 커밋(05:31Z) CI 완료 후 Deploy job이 KUBE_CONFIG를 정상 읽는지 확인. 이전 Deploy는 secret 설정 36초 전에 시작돼 skip됨.
  - 확인: `gh run list --repo ghdtjdwn/ssuMCP --limit 5`
  - 배포 완료 후: `curl https://ssumcp.duckdns.org/actuator/health`

## 다음 세션 액션 (순서)

1. **ssuMCP Deploy 확인** — `gh run list --repo ghdtjdwn/ssuMCP --limit 5` 로 최신 Deploy job 성공 여부 확인. 실패 시 로그 확인.
...
├── ssuAI/                 # Next.js 프론트엔드 (github.com/ghdtjdwn/ssuAI)
│   ├── docs/
│   │   ├── adr/           (0006 프론트스택, 0009 챗봇)
│   │   └── handoff/       (latest.md, runbook.md)
│   └── .codex/current-task.md  ("no active task")
├── ssuMCP/                # Spring Boot MCP 서버 (github.com/ghdtjdwn/ssuMCP)
│   ├── TROUBLESHOOTING.md
...
4. gh run list --repo ghdtjdwn/ssuMCP --limit 3 로 최신 Deploy 상태 확인
