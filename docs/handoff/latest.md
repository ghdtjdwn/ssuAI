...
- **Repo Split 완료**: `hoeongj/ssuAI` (모노레포) → `hoeongj/ssuMCP` (MCP 서버) + `hoeongj/ssuAI` (프론트엔드) 분리
...
1. **`KUBE_CONFIG` secret 추가** — GitHub → `hoeongj/ssuMCP` → Settings → Secrets → Actions → New secret → `KUBE_CONFIG` (기존 ssuAI repo 값과 동일)
