...
3. `app/auth/return/page.tsx`는 `code` 쿼리 파라미터가 있으면 `exchangeAuthCode` → 성공 시
   기존 `refresh()`(=/me 조회 + 인증 상태 채움) 순서로 처리하고, 어느 단계든 실패하면 기존
   "세션을 만들지 못했어요" 실패 UI를 그대로 재사용한다. 기존 `ok=1` / `lms_ok=1` 분기(레거시
   백엔드 및 LMS 리턴 경로)는 그대로 남겨 배포 전환 구간에서도 동작하게 한다.
...
- `code` 파라미터가 있으면 `exchangeAuthCode`가 정확히 1회 호출되고, 성공 시 `refresh()` → `/`로
  이동한다.
- 코드 교환이 실패하거나(예외) 교환은 성공했지만 `refresh()`가 `false`를 반환하면 실패 UI가
  뜬다.
