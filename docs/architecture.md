...
- MCP web session의 `availableProviders`가 SAINT·LMS·LIBRARY의 operational 연결 상태 기준이며, `linkedProviders`와 `providerHealth`는 grant 존재·degraded 원인·rolling compatibility를 보완한다. `UNKNOWN`은 사용할 수 있지만 `상태 미확인`으로 구분한다. 발급·만료·live grant 재검증은 앱 수준 provider가 단일 소스로 관리하며, 상태 갱신에 일시적으로 실패하면 session ID는 복구용으로 유지하되 UI는 캐시된 상태를 `stale`로 표시한다. 겹친 강제 갱신은 진행 중 요청 뒤 한 번으로 합치고, HITL action은 interrupt 당시 session에 고정한 뒤 resume stream이 끝나야 갱신한다([ADR 0099](adr/0099-authoritative-web-session-grants.md)).
