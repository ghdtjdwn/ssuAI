...
> 현재 구현 범위 문서. 기준일: 2026-05-27. 장기 방향은
...
- ssuMCP는 현재 23개 MCP 도구를 Streamable HTTP `/mcp`로 노출한다.
...
- 비밀번호, upstream 쿠키, 토큰, `mcp_session_id`, JWT, API key는 저장소나
  로그에 기록하지 않는다.
- 상태를 바꾸는 예약/취소 기능은 현재 제공하지 않는다.

## 4. 후속 범위

가장 중요한 다음 제품 단계는 도서관 좌석 예약 에이전트이다. 구현 시에는
조회 도구와 분리된 write tool, 사용자 명시 확인, 중복 실행 방지, 감사 기록,
실패/경쟁 상태 안내가 모두 갖춰져야 한다.
