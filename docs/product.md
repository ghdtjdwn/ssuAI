...
- **홈**: AI 오늘의 브리핑 히어로(연동된 데이터만으로 규칙 기반 요약 — 미연동 소스는 조용히 생략) + 우선순위 카드 3(마감 임박/지금이 적기/잊지 마세요) + 위젯 13종 커스터마이즈 그리드(순서·1/2칸·on/off·밀도, `localStorage` 영속). 학사일정 위젯은 프론트 캘린더 API 부재로 제외.
