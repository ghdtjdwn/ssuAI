...
- `/chat`은 LangGraph 멀티 에이전트(ssuAgent)와 SSE 스트리밍으로 연결되어
  자연어 질문에 답하고, 도서관 예약 등 write 액션은 HITL 승인 카드로 사용자
  확인을 받은 뒤 실행한다(2026-06-14 PR #194, [ADR 0001](adr/0001-agent-sse-chat-hitl.md)).
...
- 비밀번호, upstream 쿠키, 토큰, `mcp_session_id`, JWT, API key는 저장소나
  로그에 기록하지 않는다.
- 상태를 바꾸는 도서관 좌석 예약/이석/반납은 모두 `prepare_*` +
  `confirm_action` 2단계를 거친다. 웹 대시보드는 좌석 추천 → 예약 확정
  (`ReservationConfirmModal`) → 대기(`WaitStatusCard`) UX를 제공하며(위 §2,
  2026-06-06 PR #189 shipped), 모든 예약은 사용자가 모달에서 명시 승인해야
  confirm된다. 이석/반납과 완전 자동 예약 버튼은 아직 웹 표면으로 제공하지
  않는다.
