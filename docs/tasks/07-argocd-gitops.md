...
- **ArgoCD Image Updater** watches `ghcr.io/ghdtjdwn/ssuai-backend`, picks
...
4. **Install ArgoCD Image Updater** (`argoproj-labs/argocd-image-updater`,
   ...
   `ghcr.io/ghdtjdwn/ssuai-backend` for new `sha-<full>` tags, pick the
...
| Where | Name | Purpose |
|---|---|---|
| `argocd` namespace | `argocd-image-updater-secret` | GitHub PAT for write-back. Scope: contents:write on `ghdtjdwn/ssuAI` only. |
...
5. **Image Updater works end-to-end**:
   - Push a no-op commit to `main` → CI builds + pushes
     `ghcr.io/ghdtjdwn/ssuai-backend:sha-<new>` → Image Updater detects
...
- The Image Updater PAT is the most sensitive new artifact. It must
  ...
  `ghdtjdwn/ssuAI` only — not classic. Stored as a K8s Secret in the
...
- **ArgoCD Image Updater** watches `ghcr.io/ghdtjdwn/ssuai-backend`,
