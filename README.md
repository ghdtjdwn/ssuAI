...
| **MCP server** | 4 tools (`get_today_meal`, `get_meal_by_date`, `get_dorm_weekly_meal`, `search_campus_facilities`) over SSE. Same Spring Boot process; usable from Claude Desktop and Cursor. See [`docs/mcp-tools.md`](docs/mcp-tools.md). |
...
[`docs/mcp-tools.md`](docs/mcp-tools.md) for Claude Desktop and Cursor
wiring (direct SSE or via `mcp-proxy`).
...
Implementation history is recorded in [`docs/dev-log.md`](docs/dev-log.md),
with load-bearing decisions in [`docs/adr/`](docs/adr/).
...
- **Live deployment is still one small VM.** The public service is suitable for
  limited traffic, not high-volume production use.

---

## Contributing / project conventions

This is a one-developer project, and the repository conventions are
documented to keep changes consistent:
