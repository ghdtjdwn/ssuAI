...
별도 네이티브 앱을 두지 않고 하나의 Next.js 반응형 웹을 사용한다. `AppShell`은 Tailwind의 `lg`
분기에서 데스크톱의 246px 고정 사이드바를 모바일의 상단 바와 5개 하단 탭으로 바꾸고, 하단 탭에는
`safe-area-inset-bottom`을 적용한다. 같은 데이터와 상태 로직을 공유하면서 홈 위젯은 3열에서 1열로,
주간 시간표는 월–금 5열에서 오늘 우선 목록으로 표현만 전환한다.

모바일 브라우저에서는 `device-width`, `dvh` 기반 채팅 높이와 16px 입력창을 사용한다. 핀치 줌을
막지 않으면서 iOS 입력 포커스의 자동 확대를 피하는 방식이다. 따라서 데스크톱과 모바일은 배포물도
하나이고, 화면 폭에 따른 탐색·배치만 달라진다.

### 챗봇에서 LMS 강의자료 일괄 받기

자연어 요청을 받은 `ssuAgent`가 LMS 도메인으로 전환해 인증과 학기 목록을 확인하고, `ssuMCP`의
비동기 내보내기를 시작한다. 준비된 제한 링크는 채팅 안에서 다운로드 버튼으로만 렌더링하며,
허용된 origin과 path를 통과하지 못한 링크는 일반 텍스트로 남긴다.

![ssuAI 챗봇이 전체 LMS 강의자료 내보내기를 준비하고 다운로드 버튼을 반환한 화면](docs/assets/chat-lms-export-download.png)
...
| 모바일 홈 | 모바일 챗봇 | 모바일 학사 |
| --- | --- | --- |
| ![AI 브리핑과 우선순위 카드를 1열로 배치한 모바일 홈](docs/assets/mobile-home.png) | ![하단 탭과 추천 질문을 제공하는 모바일 챗봇](docs/assets/mobile-chat.png) | ![개인 이수값을 비식별화한 모바일 학사 화면](docs/assets/mobile-academics.png) |
