...
- **트러블슈팅 누적** — 아래 기준 중 하나라도 해당하면 대화가 끝나기 전에
  `../ssuMCP/TROUBLESHOOTING.md` 에 즉시 기록. 기억이 흐려지면 디테일이 사라진다.
  기록 기준: (1) 외부 시스템이 예상과 다르게 동작, (2) 처음 세운 가설이 틀렸고
  실제 원인이 다른 레이어, (3) 테스트 green인데 prod 깨짐, (4) 프레임워크 내부
  우회, (5) 설계 방향 전환, (6) 보안/인증/세션 버그.
  필수 포함 항목: **처음 세운 가설(틀린 방향)**, **실제 원인**, **핵심 파일/커밋**,
  **포트폴리오 포인트(왜 어려웠는지, 무엇이 non-obvious였는지)**,
  **면접 예상 질문 2-3개**. 단순 오타·일회성 명령 실패는 제외.
